...
```{r, warning = FALSE}
sdg_3_sdg_4_scatter <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_color_viridis_d() +
  labs(title = "Correlation Between SDG 3 and SDG 4",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       color = "Regions")

ggplotly(sdg_3_sdg_4_scatter)
```
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Oranges", goal_3_score)(goal_3_score), label = mytext) # Changed color to 'Oranges'
```
