...
k3 <- kmeans(sdr_data_imputed, centers = 3) # For challenge 1
```
...
country_clusters <- as.data.frame(k3$cluster)
```

**Interpretation (3 Clusters)**
At the bottom of the ``country_clusters``, you can find these:
- Lower-Middle-Income Countries & Upper-Middle-Income Countries: 1
- Low-Income Countries: 2
- High-Income Countries: 3

While countries are clustered based on how similar their SDG scores are, the country's income could influence the SDG scores. It appears that cluster 3 includes high performers (mostly rich countries), cluster 2 includes low performers (low-income countries), and cluster 1 includes moderate performers (often middle-income countries).

However, there are countries, such as Cambodia and Namibia, that are in cluster 2 but are not considered low-income. This shows that income isn't everything. There are wealthier countries that still underperform in some SDG areas. 
...
```{r}
rf_literacy <- randomForest(normalized_score_sdg4_literacy ~ .,
...
importance_df <- as.data.frame(rf_literacy$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Literacy",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
pdp::partial(rf_literacy, pred.var = "normalized_score_sdg3_uhc", plot = TRUE)
```
...
pd <- pdp::partial(rf_literacy, pred.var = c("normalized_score_sdg3_uhc",
                                             "normalized_score_sdg3_neonat"))
# Default PDP
plotPartial(pd)
```
