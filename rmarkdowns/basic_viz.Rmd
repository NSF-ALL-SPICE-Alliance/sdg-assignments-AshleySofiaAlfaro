...
```{r}
ggplot(data = sdr_data, aes(x = goal_1_score, y = goal_2_score, color = goal_3_score)) +
  geom_point()
```
Interpretation: 
SDG 1: No Poverty — SDG 2: Zero Hunger — SDG 3: Good Health & Well-Being
Countries with higher SDG 3 scores, represented by lighter blue dots, skew to the (top) right of the plot, indicating higher scores in both SDG 1 and SDG 2. This suggests that countries with lower levels of poverty and hunger generally experience better health outcomes in comparison to countries with lower SDG 1 / SDG 2 scores.
...
ggplot(mena_sdr_data, aes(x = goal_11_score, y = country)) +
  geom_bar(stat = "identity") # SDG 11: Sustainable Cities and Communities
```
...
ggplot(sdr_data, aes(x = goal_3_score, fill = regions_used_for_the_sdr)) +
  geom_histogram(bins = 50, color = "black", linewidth = 0.4)
```
Interpretation: 
Similarly, countries that have a higher SDG 3 score (between 80 and 100) belong to the OECD region, while countries in Sub-Saharan Africa fall towards the lower scores (often below 50). This suggests that wealthier, more developed regions generally perform better on health-related SDG indicators, and less developed regions face greater challenges in achieving these goals. 
